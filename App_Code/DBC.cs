...
public class DBC
{
    ...
    public static string _getParaValue(string para)
    {
        ...
        DataTable dt = DBC.getDataTable("select value from syspara where para='" + para + "'");
        if (dt.Rows.Count > 0)
        {
    ...
    public static SqlConnection createConnection()
    {
        if (sqlConnectionstr.Length.Equals(0))
        {
            try
            {
                ...
                sqlConnectionstr = ConfigurationManager.ConnectionStrings["SqlConnStr"].ConnectionString;
